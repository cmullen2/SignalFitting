...
//(same distribution as for signal) and flat for random data 
{
  ...
  RF->LoadVariable("PimTagDiffTime[-80,20]");//should be same name as variable in tree
  RooRealVar * var=RF->GetWorkSpace()->var("PimTagDiffTime");
...
//Do I need this for a farmed fit!!! No set bin dir does this!!!!
 // RF->ConfigureSavedBins("/w/work1/home/chris/LatestAnalysisRuns/Data/DataJul17/HaspectOutput/Fitting/All3Binned1FilesProtonW10/"); 
  ...
  RF->Factory("Gaussian::TimeCoinc(PimTagDiffTime, mean[-60,-80,20], SIMw[10,0,100] )");
  RF->LoadSpeciesPDF("TimeCoinc");

  RF->Factory("Chebychev::Random(PimTagDiffTime,{a0[0.1,-0.4,0.4],a1[-0.1,-0.3,0.3]})");
