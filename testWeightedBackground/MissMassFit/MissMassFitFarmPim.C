...
//  RF->LoadVariable("Coplanarity[-50,50]");//should be same name as variable in tree 
  RF->LoadVariable("MissMass[0,2040]");//should be same name as variable in tree 
...
//  RF->ConfigureSavedBins("/w/work1/home/chris/LatestAnalysisRuns/Data/DataJul17/HaspectOutput/Fitting/All3Binned1FilesProtonW10/");
  ...
  RF->Factory("RooHSEventsHistPDF::Sig(MissMass,alpha[0,0,100],off[0,-2,2],scale[1,0.6,1.4])");
  ...
  RF->Factory("RooHSEventsHistPDF::BG(MissMass,alphaB[0,0,20],offB[0,-2,2],scaleB[1.0,0.6,1.4])");
